...
const showFooter = computed(() => formatter.value.footer !== false)
...
<template>
  <div>
    ...
    >
      <div
        class="clip bg-gradient-to-r from-[#0e5f52] to-[#123a5e]"
        :style="{ 'clip-path': `polygon(${poly1})`, 'opacity': opacity }"
      />
      <div
        class="clip bg-gradient-to-l from-[#16805f] to-[#0e2f56]"
        :style="{ 'clip-path': `polygon(${poly2})`, 'opacity': opacity }"
      />
      <div
        class="clip bg-gradient-to-t from-[#25c996] to-[#b0ddff]"
        :style="{ 'clip-path': `polygon(${poly3})`, 'opacity': 0.2 }"
      />
    </div>

    <div v-if="showFooter" class="deck-footer" aria-hidden="true">
      <span class="deck-tag">#KubeCon #CloudNativeCon</span>
      <img src="/kubecon-logo.svg" class="deck-logo">
    </div>
  </div>
</template>
